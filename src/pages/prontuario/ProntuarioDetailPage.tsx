...
interface HistoricoUnificadoItem {
    ...
    data: string; // Data específica do registro
    dataOriginal: any;
    titulo?: string;
...
const ProntuarioDetailPage: React.FC = () => {
    ...
    const historicoUnificado = useMemo(() => {
        ...
        prontuario.consultas?.forEach(c => itens.push({
            id: `consulta-${c.id}`, tipo: 'CONSULTA', data: c.dataConsulta, dataOriginal: c, titulo: `Consulta`, icone: <Activity className="h-5 w-5" />
        }));
...
const ListaDeConsultas: React.FC<{consultas: ConsultaDetalhada[], formatDateTime: Function, formatDate: Function}> = ({consultas, formatDateTime, formatDate}) => {
    ...
    const itens: HistoricoUnificadoItem[] = consultas.map(c => ({id: `consulta-list-${c.id}`, tipo: 'CONSULTA', data: c.dataConsulta, dataOriginal:c, titulo: 'Consulta', icone: <Activity className="h-5 w-5" />}));
